...
public class SphereGenerator : MonoBehaviour
{
    ...
    public int numberOfHexagons; //set manually in the inspector (2 - 30, 3- 150, 4-240)
    public int result;
    ...
    Mesh GenerateIcosphere(int subdivisions)
    {
        ...
        // Define initial triangles of the icosahedron
    ...
    {
        // Get or add the midpoint between two vertices
        ...
        if (!midpointCache.ContainsKey(key))
        {
            ...
            // Create an empty GameObject at the midpoint
    ...
    void CreateEmptyObject(Vector3 position, string name)
    {
        ...
        // Calculate the normal vector at the specified position
        Vector3 normal = position.normalized;

        // Rotate the emptyObject to align with the normal vector
        emptyObject.transform.rotation = Quaternion.FromToRotation(Vector3.up, normal);

        // Additional rotation to align hexagons properly
        emptyObject.transform.RotateAround(Vector3.zero, normal, rotation*(1f-emptyObject.transform.position.y)); // Adjust the angle as needed
    }
    ...
    int GetMidpoint(List<Vector3> vertices, int v1, int v2, int v3) //midpoint between 3 vertices for calculating the center of the triangles
    {
